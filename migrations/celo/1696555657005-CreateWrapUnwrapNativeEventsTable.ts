...
const createWrapNativeEventIndexQuery = `
    ...
    ON events_celo.wrap_native_events (transaction_hash);

    CREATE INDEX wrap_native_events_block_number_index
    ON events_celo.wrap_native_events (block_number);
`;

const dropWrapNativeEventIndexQuery = `
    DROP INDEX events_celo.wrap_native_events_transaction_hash_index;
    DROP INDEX events_celo.wrap_native_events_block_number_index;
`;
...
const createUnwrapNativeEventIndexQuery = `
  ...
  ON events_celo.unwrap_native_events (transaction_hash);

  CREATE INDEX unwrap_native_events_block_number_index
  ON events_celo.unwrap_native_events (block_number);
`;

const dropUnwrapNativeEventIndexQuery = `
  DROP INDEX events_celo.unwrap_native_events_transaction_hash_index;
  DROP INDEX events_celo.unwrap_native_events_block_number_index;
`;

export class CreateWrapUnwrapNativeEventsTable1696555657005 implements MigrationInterface {
