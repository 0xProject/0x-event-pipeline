...
const wrapNativeEvent = new Table({
    name: 'events.wrap_native_events',
    columns: [
...
const createWrapNativeEventIndexQuery = `
    ...
    ON events.wrap_native_events (transaction_hash);

    CREATE INDEX wrap_native_events_block_number_index
    ON events.wrap_native_events (block_number);
`;

const dropWrapNativeEventIndexQuery = `
    DROP INDEX events.wrap_native_events_transaction_hash_index;
    DROP INDEX events.wrap_native_events_block_number_index;
`;

const unwrapNativeEvent = new Table({
    name: 'events.unwrap_native_events',
    columns: [
        { name: 'observed_timestamp', type: 'bigint' },
        { name: 'contract_address', type: 'varchar' },
        { name: 'transaction_hash', type: 'varchar', isPrimary: true },
...
const createUnwrapNativeEventIndexQuery = `
  ...
  ON events.unwrap_native_events (transaction_hash);

  CREATE INDEX unwrap_native_events_block_number_index
  ON events.unwrap_native_events (block_number);
`;

const dropUnwrapNativeEventIndexQuery = `
  DROP INDEX events.unwrap_native_events_transaction_hash_index;
  DROP INDEX events.unwrap_native_events_block_number_index;
`;

export class CreateWrapUnwrapNativeEventsTable1696555657000 implements MigrationInterface {
