...
const wrapNativeEvent = new Table({
    name: 'events_polygon.wrap_native_events',
    columns: [
...
const createWrapNativeEventIndexQuery = `
    ...
    ON events_polygon.wrap_native_events (transaction_hash);

    CREATE INDEX wrap_native_events_block_number_index
    ON events_polygon.wrap_native_events (block_number);
`;

const dropWrapNativeEventIndexQuery = `
    DROP INDEX events_polygon.wrap_native_events_transaction_hash_index;
    DROP INDEX events_polygon.wrap_native_events_block_number_index;
`;

const unwrapNativeEvent = new Table({
    name: 'events_polygon.unwrap_native_events',
    columns: [
        { name: 'observed_timestamp', type: 'bigint' },
        { name: 'contract_address', type: 'varchar' },
        { name: 'transaction_hash', type: 'varchar', isPrimary: true },
...
const createUnwrapNativeEventIndexQuery = `
  ...
  ON events_polygon.unwrap_native_events (transaction_hash);

  CREATE INDEX unwrap_native_events_block_number_index
  ON events_polygon.unwrap_native_events (block_number);
`;

const dropUnwrapNativeEventIndexQuery = `
  DROP INDEX events_polygon.unwrap_native_events_transaction_hash_index;
  DROP INDEX events_polygon.unwrap_native_events_block_number_index;
`;

export class CreateWrapUnwrapNativeEventsTable1696555657008 implements MigrationInterface {
