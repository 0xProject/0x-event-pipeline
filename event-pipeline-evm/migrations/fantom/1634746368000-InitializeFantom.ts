...
const downQuery = `
DROP SCHEMA events_fantom CASCADE;
`;

const upQuery = `
create table events_fantom.blocks
(
...
create index blocks_block_timestamp_index
  on events_fantom.blocks (to_timestamp(block_timestamp::double precision));

create table events_fantom.last_block_processed
(
...
create table events_fantom.erc20_bridge_transfer_events
(
  observed_timestamp bigint not null,
...
create index bridge_trades_transaction_hash_index
  on events_fantom.erc20_bridge_transfer_events (transaction_hash);

create index bridge_trades_block_number_index
  on events_fantom.erc20_bridge_transfer_events (block_number);

create index bridge_trades_contract_address_index
  on events_fantom.erc20_bridge_transfer_events (contract_address);

create table events_fantom.transactions
(
  observed_timestamp bigint not null,
  transaction_hash varchar not null
...
create index transactions_block_number_index
  on events_fantom.transactions (block_number);

create table events_fantom.transformed_erc20_events
(
  observed_timestamp bigint not null,
  contract_address varchar not null,
  transaction_hash varchar not null,
...
create index transformed_erc20_transaction_hash_index
  on events_fantom.transformed_erc20_events (transaction_hash);

create index transformed_erc20_block_number_index
  on events_fantom.transformed_erc20_events (block_number);

create index transformed_erc20_taker_index
  on events_fantom.transformed_erc20_events (taker);

create table events_fantom.transaction_receipts
(
  observed_timestamp bigint not null,
  transaction_hash varchar not null
...
create index transaction_receipts_block_number_index
  on events_fantom.transaction_receipts (block_number);

create table events_fantom.transaction_logs
(
  observed_timestamp bigint not null,
  transaction_hash varchar not null
...
create table events_fantom.v4_rfq_order_filled_events
(
  observed_timestamp bigint not null,
  contract_address varchar not null,
  transaction_hash varchar not null,
...
create index rfq_order_fills_v4_transaction_hash_index
  on events_fantom.v4_rfq_order_filled_events (transaction_hash);

create index rfq_order_fills_v4_block_number_index
  on events_fantom.v4_rfq_order_filled_events (block_number);

create index rfq_order_fills_v4_maker_index
  on events_fantom.v4_rfq_order_filled_events (maker);

create table events_fantom.v4_limit_order_filled_events
(
  observed_timestamp bigint not null,
  contract_address varchar not null,
  transaction_hash varchar not null,
...
create index limit_order_fills_v4_transaction_hash_index
  on events_fantom.v4_limit_order_filled_events (transaction_hash);

create index limit_order_fills_v4_block_number_index
  on events_fantom.v4_limit_order_filled_events (block_number);

create index limit_order_fills_v4_maker_index
  on events_fantom.v4_limit_order_filled_events (maker);

create table events_fantom.native_fills
(
  observed_timestamp bigint not null,
  contract_address varchar not null,
  transaction_hash varchar not null,
...
create index native_fills_transaction_hash_index
  on events_fantom.native_fills (transaction_hash);

create index native_fills_block_number_index
  on events_fantom.native_fills (block_number);

create index native_fills_maker_index
  on events_fantom.native_fills (maker);
`;

export class InitializeFantom1634746368000 implements MigrationInterface {
