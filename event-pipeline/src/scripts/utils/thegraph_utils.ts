...
export class PullAndSaveTheGraphEvents {
    public async getParseSaveUniswapSwapsAsync(
        ...
        await this._deleteOverlapAndSaveSwapEventsAsync(
            connection,
            parsedSwaps,
            firstBlockNumber,
            protocol,
    ...
    private async _getStartTimestampAsync(
        ...
        return Math.min(
            Number(lastKnownBlock.last_processed_block_timestamp) + 1,
            latestBlockTimestampWithOffset - START_BLOCK_TIMESTAMP_OFFSET,
        );
    }
    ...
    private async _deleteOverlapAndSaveSwapEventsAsync(
        ...
        try {
            ...
            await queryRunner.manager.query(
                `DELETE FROM events.erc20_bridge_transfer_events WHERE block_number >= ${startBlock} AND direct_protocol = '${protocol}'`,
            );
