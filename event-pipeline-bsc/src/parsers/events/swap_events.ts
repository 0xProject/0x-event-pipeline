...
export function parsePancakeSwapEvent(eventLog: RawLogEntry): ERC20BridgeTransferEvent {
    ...
    eRC20BridgeTransferEvent.fromToken =
        new BigNumber(decodedLog.amount0In) > new BigNumber(decodedLog.amount0Out) ? '0' : '1'; // taker_token
    eRC20BridgeTransferEvent.toToken =
        new BigNumber(decodedLog.amount0In) > new BigNumber(decodedLog.amount0Out) ? '1' : '0'; // maker_token
    ...
    eRC20BridgeTransferEvent.fromTokenAmount = amount0In.gt(amount0Out) ? (amount0In-amount0Out) : (amount1In-amount1Out); // taker_token_amount
    eRC20BridgeTransferEvent.toTokenAmount = amount0In.gt(amount0Out) ? (amount1Out-amount1In) : (amount0Out-amount0In); // maker_token_amount
