...
export function parsePancakeSwapEvent(eventLog: RawLogEntry): ERC20BridgeTransferEvent {
    ...
    eRC20BridgeTransferEvent.fromToken =
        new BigNumber(decodedLog.amount0In) > new BigNumber(decodedLog.amount1In) ? '0' : '1'; // taker_token
    eRC20BridgeTransferEvent.toToken =
        new BigNumber(decodedLog.amount0In) > new BigNumber(decodedLog.amount1In) ? '1' : '0'; // maker_token
